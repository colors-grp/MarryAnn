...
function main(){
	gameCanvas = document.getElementById("gameCanvas");
        ...
        //alert(BOARD_HEIGHT + ' X ' + BOARD_WIDTH);
...
function toggleClickEvent(on){
	if(on){
		...
		gameCanvas.addEventListener("touchstart", selectToken, false);
		gameCanvas.addEventListener("touchend", selectToken, false);
	}else{
		//gameCanvas.removeEventListener("click", selectToken, false);
		gameCanvas.removeEventListener("touchstart", selectToken, false);
	}
}
...
function getCell(e){
        ...
        if(startX != -1 && startY != -1){// another cell has been selected
        	if(x > startX + (TOKEN_SIZE/4)){
        		col = 1;
        	} else if(x < startX - (TOKEN_SIZE/4)){
        		col = -1;
        	} else if (y > startY + (TOKEN_SIZE/4)){
        		row = 1;
        	} else if(y < startY - (TOKEN_SIZE/4)){
        		row = -1;
        	}
        	//alert(startX + "   " + startY + "\n" + x + "  " + y + "\n" + col + "  " + row);
        	alert('end');
        	return (new Cell(startCellX + col, startCellY + row ));	
        } else {
        	return (new Cell(Math.floor(x / (TOKEN_SIZE+SPACE)) + col, Math.floor(y / (TOKEN_SIZE+SPACE)) + row ));	
        }
        ...
        if(startX != -1 && startY != -1){// another cell has been selected
        	alert(startX + "   " + startY + "\n" + x + "  " + y + "\n" + col + "  " + row);
        	// Math.floor(x / (TOKEN_SIZE+SPACE) + "  " + Math.floor(y / (TOKEN_SIZE+SPACE)));
...
function selectToken(e){
	...
	// Out of bound?
	if(selectedCell.col >= TOKEN_PER_COL || selectedCell.row >= TOKEN_PER_ROW || selectedCell == 'undefined'){
		return;
	}
	
	if (isSelectingFirst) {
		startCellX = selectedCell.col;
		startCellY = selectedCell.row;
		var rect = gameCanvas.getBoundingClientRect();
