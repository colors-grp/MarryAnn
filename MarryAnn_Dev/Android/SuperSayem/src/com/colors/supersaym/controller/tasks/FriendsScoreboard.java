...
public class FriendsScoreboard extends Task {
	private String url;
	Context mxontext;
	...
	public FriendsScoreboard(DataRequestor rquestor, Context context,
			String UserId, String CategoriD, String CatNAme, int start, int size,String accountId) {
		setRequestor(rquestor);
		setId(TaskID.FriendsScoreTask);
		scoreBoardArray = new ArrayList<UserData>();
		this.mxontext = context;
		url = Communication.Base_URL
				+ "friends_scoreboard/format/json/facebookId/"+UserId+"/categoryId/" + CategoriD
				+ "/categoryName/" + CatNAme + "/start/" + start + "/size/"
