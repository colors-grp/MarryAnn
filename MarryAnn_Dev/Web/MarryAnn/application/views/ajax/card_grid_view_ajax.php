...
<div id="list_view">
	<table id="cards" style="text-align: left;">
		<?php
                $count = 0;
                $card_time = '2014-06-29 22:00:00';
                $bouns_cards = array('33','34','35');
//            if(!in_array($_SESSIO
		$i = 0;
		$first_blocked_card = false;
                if(count($blocked_cards) > 0){
                    if(!in_array($blocked_cards[0]['id'], $bouns_cards)){
                        $i++;
                        $count++;
		...
		foreach (array_reverse($cards) as $card) {
                    if($cat_id == 1 && in_array($card['id'], $bouns_cards)){continue;}
			if ($i % 2 == 0) {
                            if ($i != 0) { ?>
                            </tr>
                            <?php } ?>
