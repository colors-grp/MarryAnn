 ...
 */
var score = 0;
var time_interval_match3, timer=60, timer_speed=1000, game_started=true,over_triggered=false;
var matches, hints, frame_active=false,combo=0,moves=0;
//========================================================
// MAIN METHODS
//========================================================
function main(){
        ...
        moves=0;
        game_started = true;
        //Set Show initial score and timer to user
        score=0;
	...
	gameCanvas = document.getElementById("gameCanvas");
        ...
        gameCanvas.height = CELL_SIZE * TOKEN_PER_COL;
        gameCanvas.width  = CELL_SIZE * TOKEN_PER_ROW;
	...
	gridCanvas = document.getElementById("gridCanvas");
        // Change grid canvas size
        gridCanvas.height = CELL_SIZE * TOKEN_PER_COL;
        gridCanvas.width  = CELL_SIZE * TOKEN_PER_ROW;
	gridContext = gridCanvas.getContext("2d");
	
	gridContext.fillStyle = "#663";
...
//	board[3][4].special = new MoonToken();
        //setting time intervales.
        clearInterval(time_interval_match3);
        time_interval_match3 = setInterval('count_timer()', timer_speed);
}

function count_timer(){
//    alert('fram_active='+frame_active);
    if(timer <= 0){//Stop the game ????
        clearInterval(time_interval_match3);
        game_started = false;
        if(!frame_active){
...
//	}
//        alert(gameCanvas.offsetLeft+'\n'+gameCanvas.offsetTop);
//	alert(x+'\n'+y);
//        alert(Math.floor(x / (TOKEN_SIZE+SPACE))+'\n'+Math.floor(y / (TOKEN_SIZE+SPACE)));
...
function checkMatches(callback){
	...
	if(matchLists.length > 0){
		//alert(matchLists.join('\n'));
//                alert('combo_triggired='+combo_triggired);
                for(i = 0; i < matchLists.length; i++){
                    combo++;
                    var tempScore = 10 * (matchLists[i].length - 2) * combo;
                    increment_score(10 * (matchLists[i].length - 2) * combo);
//                    alert(matchLists[i].length);
//                    alert('score+='+tempScore);
                }
//                combo+=matchLists.length;
//                score += (20 * combo);
//                alert('combo='+combo);
...
//                alert('no');
        ...
        matches = findMatches();
        hints = getHintList();
        if(!game_started && matches.length == 0){
            game_over();
        }
}
	...
	(function wait(){
		...
		if (f < frame){
			requestAnimationFrame(wait);
                    ...
                    if(hints.length > 0 || matches.length > 0){
                        if (game_started){
                            toggleClickEvent(true);
                            toggleMouseMoveEvent(true);
                        }
...
// Increment score with given value
function increment_score(number){
	score += number;
	//alert(score);
	document.getElementById("score").innerHTML = score;
}
...
function game_over(){
    if(!over_triggered){
        ...
        clearInterval(time_interval_match3);
        document.getElementById("score").innerHTML = score;
//        var ajaxpage = document.getElementById('base_url').innerHTML + "index.php?/game_center/update_score";
//        $.post(ajaxpage , {game_score: score}).done(function( data ) {});
