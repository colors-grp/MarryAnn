...
//			
//==============================================================================
var score = 300, found=0, timer_speed=1000, wait_time = 20000/*20 seconds*/, game_started=true, time_interval_word_search;
(function( $, undefined ) {
		...
		$(w).css("background",'yellow').animate({"opacity": 'hide'},1000,"linear",
					 function () {
						 ...
						 $(w).addClass('rf-foundword').animate({"opacity": 'show'},1000,"linear")
                                         ...
                                         found+=1;
                                         if($('.rf-ptrue').length <= found){
                                             clearInterval(time_interval_word_search);
...
function Grid() {
   ...
   //set empty cells
   var arabic_alpha = ["ا", "ب", "ت", "ث", "ج", "ح", "خ", "د", "ذ", "ر", "س", "ش", "ص", "ض", "ط",  "ظ", "ع", "غ", "ف", "ق", "ك", "ل", "م", "ن" ,"ه", "و", "ى"];
    //alert(String.fromCharCode(9632));
	for (var i=0;i<this.size();i++) {
        for (var j=0;j<this.size();j++) {
            if (this.cells[i][j].isUnwritten()) {
                this.cells[i][j].value = arabic_alpha[Math.floor(Math.random()*27)];//String.fromCharCode(Math.floor(65+Math.random()*26));
            }
        }
    }
...
function first_timer(){
    clearInterval(time_interval_word_search);
    time_interval_word_search = setInterval('count_timer()', timer_speed);
